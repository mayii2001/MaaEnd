...
从 zmdmap 精简游戏数据生成 OutpostTrading 任务的 Pipeline、Task 选项与部署数据。任务流程、选品/干员规则和缓存语义见[开发手册 - OutpostTrading](../../../docs/zh_cn/developers/tasks/outpost-trading-maintain.md)，本文只讲生成器本身。
...
- `model.mjs`：共享模型，统一定义据点/地区、多语言键和稳定顺序。据点 `LocationId` 由游戏数据英文名称派生；只有存在实际 OCR 误识证据时才追加识别候选。地区顺序在这里定义，生成器按 `outpostTradingRegionsNewestFirst`（新地区优先）输出主循环遍历列表，地区内据点保持模型顺序。
...
| `sell-data.mjs` | 地区售卖入口（含地区据点锚点 `OutpostTradingIn{Region}Outpost`） |
