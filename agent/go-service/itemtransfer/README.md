...
当前 ItemTransfer Pipeline 使用 C++ `IconRecognition` 查找物品，由 Go Service 对候选格进行分类反查。页面滚动、仓库切换和转移动作仍由 Pipeline 控制。

## Go 组件

- `ItemTransferValidatedIconRecognition`：识别当前页目标物品候选，并逐格反查实际物品；
...
`ItemTransferValidatedIconRecognition` 接收 Task 覆盖注入的唯一 `item_id` 和 `item_filter`：
...
- `item_ids`：目标物品 ID；
- `item_filters`：由 catalog 的 `storageKind` 和 `categoryType` 组成；
...
四个节点保留各自原有 ROI、锚点和点击目标，仅把 Custom Recognition 注册名设置为 `ItemTransferValidatedIconRecognition`。识别未命中时继续使用 `ItemTransferRepoSearchCurrentPage` 或 `ItemTransferBagSearchCurrentPage` 的既有滚动逻辑。

## 文件结构

```text
agent/go-service/itemtransfer/
├── validated_icon_recognition.go  # 当前页候选识别与分类反查
