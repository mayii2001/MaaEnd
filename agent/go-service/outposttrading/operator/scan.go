...
func loadOperatorOwnershipForSelection() (operatorOwnership, error) {
	uid := currentOutpostTradingCacheUID()
	path := resolveOutpostTradingCachePathFunc()
...
func operatorCacheStatusForSelection(p *operatorRecognitionParam) (operatorCacheStatus, error) {
	...
	if !outpostTradingCacheHasOperatorSnapshot(cache, uid) {
...
func shouldWriteOperatorCacheSnapshot(
	p *operatorRecognitionParam, cache outpostTradingCache,
	uid string,
) bool {
	...
	return p.Mode == operatorCacheModeCache && !outpostTradingCacheHasOperatorSnapshot(cache, uid)
}
