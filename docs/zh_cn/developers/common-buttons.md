...
| 项目 | 说明 |
| ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| **识别方式** | `And`：先用 `SceneManager/MapMissionReminder.png` 确认处于地图界面，再在 ROI `[1160, 600, 120, 120]`（720p）内匹配白色确认按钮模板（普通态与 Hover 态）。 |
| **搜索范围** | **限定区域** ROI `[1160, 600, 120, 120]`（720p），且仅在地图界面生效。 |
| **动作** | 单击。 |
| **使用条件** | 地图界面中需要点击右下角传送按钮时使用。若按钮位置不同（如快速传送按钮），改用 `QuickTeleportButton`。 |

---

## QuickTeleportButton

**说明**：前往传送按钮（快速传送），位于地图界面右下角传送按钮的左侧。

<!-- 截图：tests/MaaEndTestset/Win32/Official_CN/武陵_地图_拍摄巨兽心脏_前往传送.png -->
...
| 项目 | 说明 |
| ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **识别方式** | 与 `TeleportButton` 相同，按钮区 ROI 为 `[1000, 600, 120, 120]`（720p），阈值 0.87。 |
| **搜索范围** | **限定区域** ROI `[1000, 600, 120, 120]`（720p），且仅在地图界面生效。 |
| **动作** | 单击。 |
| **使用条件** | 地图界面中需要点击「前往传送」（点击后地图视窗移动到目标传送点）时使用，之后通常还需再点击 `TeleportButton` 完成传送。 |
...
| `TeleportButton` | 地图界面右下角的传送按钮 |
| `QuickTeleportButton` | 地图界面中传送按钮左侧的快速传送按钮 |
