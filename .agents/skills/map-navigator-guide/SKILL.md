...
除 `ZONE` 区域声明外，`path` 中的节点默认都是可选路线 / 动作提示：运行时先尝试从区间起点直达末端位置，成功时跳过中间的 `NAVMESH`、录制 `RUN`、`TRANSFER`、`PORTAL`、`INTERACT` 等点及其动作，失败才按完整作者路径逐段执行。确实必须抵达并执行的节点写 `required: true`，它会切断区间并成为后续规划的新起点。跨越多张面的长距离仍可逐段给提示点，用它们为直达失败提供可达性兜底。
