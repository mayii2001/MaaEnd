...
try {
    ...
    console.error("[OutpostTrading] 数据文件缺失，请先运行 pnpm fetch:zmdmap 或 pnpm generate:OutpostTrading");
    process.exit(1);
}
...
export const outpostTradingLocations = Object.entries(settlementData.settlements)
    .sort(
        (
            [
                aId,
...
// 区域级售卖入口消费的最小模型，区域和据点顺序与 outpostTradingLocations 保持一致。
export const outpostTradingRegions = Object.values(
    outpostTradingLocations.reduce((regions, location) => {
...
export const outpostTradingRegionsNewestFirst = [...outpostTradingRegions].reverse();
const outpostTradingLocationById = new Map(
    outpostTradingLocations.map((location) => [
        location.LocationId,
        location,
    ]),
);
export const outpostTradingLocationsNewestFirst = outpostTradingRegionsNewestFirst.flatMap((region) =>
    region.LocationIds.map((locationId) => outpostTradingLocationById.get(locationId)),
);

// 国际化同步器消费的最小数据视图。命名规则与所有模板共享同一模型。
export const outpostTradingLocaleEntries = {
    ...
    settlements: outpostTradingLocations.map((location) => ({
        key: `task.OutpostTrading.${location.RegionPrefix}${location.LocationId}`,
