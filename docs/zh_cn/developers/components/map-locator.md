...
| `camRot` | 摄像机朝向，0°–360°，北为零向，仅定位成功帧携带（机制见[定位原理](#定位原理)）。与 `rot`（角色朝向）无关联 |
| `camRotConf` | 摄像机朝向置信度 |
...
5. **摄像机朝向工件**：`camRot` / `camRotConf` 由 `assets/resource/model/map/cameraorientation/` 下同一批交付的两图工件输出。`preprocess.onnx` 是前处理的唯一实现：输入 118×120 观测 ROI（BGR）、zone 底图资产（BGRA，尺寸动态）与定位 `(x, y, scale)`，输出观测条带与参考条带；极坐标几何、参考采样与条带域合成、采样与取整约定全部封装在图内，参考 BGR 在资产透明处按白底合成、越界读 0，参考 alpha 保留资产原始值。`polar_with_ref.onnx` 消费 7 通道 `[obs.BGR, ref.BGR, ref.A]` 参考配对，输出 360 bin 方位角概率分布。参考缺失（裁剪越界或资产透明，`ref.A` 为 0）已在配对里显式表达、由模型自行处理，故 MapLocator 侧不选路也不回退：底图资产缺失或非 BGRA 时喂全透明占位资产，参考条带即为全缺失，缺口占比只进诊断日志。参考配对需要定位结果 `(x, y, zone)`，因此只有定位成功且 zone 非 `None` 的帧有 `camRot`；模型未加载或推理失败时不输出。
